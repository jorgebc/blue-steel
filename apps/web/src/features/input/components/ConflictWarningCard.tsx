...
export function ConflictWarningCard({ conflict, acknowledged, onAcknowledge }: Props) {
  ...
  return (
    <div role="alert" className="rounded-2xl border border-amber-200 bg-amber-50 p-6">
      ...
      <p className="mb-3 text-sm text-amber-900">{conflict.description}</p>
      <dl className="mb-4 space-y-1 text-sm">
        <div className="flex gap-2">
          <dt className="text-amber-700">This session</dt>
          <dd className="text-amber-900">{conflict.extractedFact}</dd>
        </div>
        <div className="flex gap-2">
          <dt className="text-amber-700">World state</dt>
          <dd className="text-amber-900">{conflict.existingFact}</dd>
        </div>
      </dl>
      <div className="flex items-center gap-2">
        ...
        />
        <Label htmlFor={ackId} className="text-amber-900">
