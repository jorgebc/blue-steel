...
Roadmap status: Phases 0–4 of the v1 roadmap (infrastructure, auth, campaigns, session ingestion, Query Mode, Exploration Mode) are **complete**. **v2 is now feature-complete** — **Phase 5** (Proposals & Approval Pipeline — see [proposals.md](proposals.md)), **Phase 6** (Input & Query enhancements: commit `add` action + Q&A history log), **Phase 7** (Campaign Export — see [Deferred & Planned Features](deferred_and_planned.md)), **Phase 8** (User Profile, Settings, Theme & UI i18n — see [system_platform.md](system_platform.md)), and **Phase 9** (Per-Campaign Content Language — see [campaign_management.md](campaign_management.md)) have all shipped.
