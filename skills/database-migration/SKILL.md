...
<addColumn tableName="entity_embeddings">
    <column name="embedding" type="vector(1536)">
        <!-- 1536 dimensions for gemini-embedding-001 with outputDimensionality=1536 (D-093) -->
        <constraints nullable="false"/>
    </column>
</addColumn>
```
...
- **Using the wrong dimension count for embeddings.** Blue Steel uses `gemini-embedding-001`
  with `outputDimensionality=1536` (D-093). The column type must be `vector(1536)`. Using a different
